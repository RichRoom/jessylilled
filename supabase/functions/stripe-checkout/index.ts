import 'jsr:@supabase/functions-js/edge-runtime.d.ts';import Stripe from 'npm:stripe@17.7.0';import { createClient } from 'npm:@supabase/supabase-js@2.49.1';const supabase = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');const stripeSecret = Deno.env.get('STRIPE_SECRET_KEY')!;const stripe = new Stripe(stripeSecret, {  appInfo: {    name: 'Bolt Integration',    version: '1.0.0',  },});
...
Deno.serve(async (req) => {
  try {
    ...
    if (!user.email || user.email.trim() === '') {
      ...
      return corsResponse({ 
        error: 'User email is required for creating Stripe customer',
        debug: { email: user.email, type: typeof user.email }
      }, 400);
    }
